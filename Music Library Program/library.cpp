...
Artist *create_artist_instance(char *name){ //create 기본값으로 해준 뒤, artist의 주소값 반환
...
Artist *add_artist(char *name){
    ...
    if (p==NULL && q==NULL){ // unique node
        ...
        ptr_artist -> next = artist_directory[(unsigned char)name[0]]; //==p
        artist_directory[(unsigned char)name[0]] = ptr_artist;
    }
    else { //add after(q)
        ptr_artist -> next = p;
    ...
    return ptr_artist; // ptr_artist에 대한 포인터 반환
...
void insert_to_index_directory(Song *ptr_song){
    ...
    ptr_snode -> song = ptr_song;
    ptr_snode->next = NULL;
    ptr_snode->prev = NULL; // unused(단방향)
...
void insert_node(Artist *ptr_artist, SNode *ptr_snode){ //노래
    SNode *p = ptr_artist->head;
    while (p!=NULL && strcmp(p->song->title, ptr_snode->song->title) < 0) //연결리스트에 저장되어있는 노래제목, 삽입하련느 노래제목
        p = p->next;

    // add ptr_snode before p
    // 1. empty 2. at the front 3. at the end 4. in the middle
    //p가 null이라는 것이 꼭 연결리스트가 empty라는 것을 가르키는 것은 아님.
...
Artist *find_artist(char *name){
    Artist *p = artist_directory[(unsigned char)name[0]]; //first artists with initial name[0]
    while (p != NULL && strcmp(p->name, name) < 0) //알파벳순
        p = p->next;
...
void status(){
    for (int i = 0; i < NUM_CHARS; i++){   
        Artist *p = artist_directory[i]; // 단방향 연결리스트의 head node의 시작 주소
        while (p!=NULL){
            print_artist(p);
            p = p->next;
        }
...
void remove(int index){
    ...
    while (p-> next != NULL && p->song->index != index) {
        q = p;
        p = p->next;
    }
